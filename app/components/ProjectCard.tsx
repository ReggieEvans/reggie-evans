...
const ProjectCard: React.FC<ProjectCardProps> = ({ name,
  ...
  return (
    <div className="bg-dark shadow-lg rounded-md p-6 w-80">
      ...
      <h3 className="text-2xl font-black uppercase text-[#F2F7F2] mt-4 blue_gradient">{name}</h3>
      <p className="text-[#F2F7F2] mt-2 opacity-90">{description}</p>

      <div className='h-[1px] w-full bg-[#F2F7F2] opacity-10 my-4'></div>

      {status && status === "In Progress" ? (
        <div className='h-36 text-4xl font-black text-blue opacity-10 uppercase py-8'>In Progress</div>
      ) : (
        <>
      ...
      <Link href={moreDetailsLink} className="flex items-center justify-between mt-4">
        <button
          className="text-white w-full bg-gradient-to-br from-pink to-red hover:brightness-125 focus:ring-2 focus:outline-none focus:ring-blue rounded-md px-5 py-2.5 text-center me-2 mb-2 text-md min-w-[250px] shadow cursor-pointer transition-all duration-300"
        >
          More Details
        </button>
      </Link>
      ...
      <div className="flex justify-center items-center gap-3 mt-4 text-xl">
        {stack?.map((tech, index) => {
          ...
          return (
            <div key={index} className="group relative flex items-center justify-center bg-black rounded-full p-2">
